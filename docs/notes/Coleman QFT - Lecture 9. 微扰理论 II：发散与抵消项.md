...
>
> 另一方面，让我们考虑 $t=-\infty$ 时的真空态 $|0\rangle$ 的演化。对于足够小的正数 $\epsilon$ ，源加上来的速度是足够慢的，由**绝热定理**，我们知道 $t=0$ 时的态即源存在下的基态 $|0\rangle_P$。但是，在 $t=0$ 时源被突然撤去，因此态仍将保持在 $|0\rangle_P$ 上（注意我们在相互作用绘景中）。也就是说，我们只需要计算 $S=U_I(\infty,-\infty)$，将其作用于真空态 $|0\rangle$ 就能得到 $|0\rangle_P$。即
>
> $$U_I(\infty,-\infty)|0\rangle=|0\rangle_P$$
> 
